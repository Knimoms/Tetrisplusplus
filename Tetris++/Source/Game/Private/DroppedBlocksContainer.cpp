...
void DroppedBlocksContainer::AddTetromino(Tetromino* addingTetromino)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        m_ColorMatrix[(unsigned int)(position[1] + blockOffsets[i][1])][(unsigned int)(position[0] + blockOffsets[i][
            0])] = {1.f, addingTetromino->GetColor()};
    }
...
void DroppedBlocksContainer::StartRemoveAnimation(int rowY)
{
    ...
    Game::GetGameInstance().GetRenderer()->AddRenderEntry(m_AnimMesh.get(), m_AnimMesh.get(), &m_AnimTransforms[i],
                                                          m_AnimShader.get(), 2);
}
